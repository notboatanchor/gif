...
| **Per-scope audit chains** (parallel append throughput) | Write volume beyond the single-chain serialized ceiling. Chain writes currently serialize per month partition (migration 016), which preserves linkage integrity under concurrent writers and is not a practical constraint at human scale; per-scope chains lift the throughput ceiling for multi-tenant volume but change the canonical audit-record form | Medium-term — trigger is real multi-tenant write demand |

The first four items are prerequisites for a Big 4 auditor to rely on GIF's audit trail in a SOC 2 Type II engagement. The next two are prerequisites for deployment in the highest-sensitivity regulated contexts. With one exception, none require architectural changes to GIF — they are additive controls built on the existing foundation. The exception is per-scope audit chains, which change the canonical audit-record form and are deliberately gated on real multi-tenant demand for exactly that reason.
