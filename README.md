...
The RC line tracked MCP SDK 2.0 prereleases through the spec RC window. The MCP SDK finalized at `2.0.0` on 2026-07-27; `main` pins `2.0.0` and hosts via the SDK's `createMcpHandler` layer, serving the `2026-07-28` protocol revision (`server/discover`, cache envelopes) to modern clients and 2025-era clients via the SDK's built-in stateless fallback. This ships as `v0.2.0` final.
