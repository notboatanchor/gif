...
export async function executeSessionClose(
  ...
  let rejection: CloseRejection | null = null;
  let auditSessionId: string | null = null;

  try {
    const result = await pool.query<{ persona_id: string; ended_at: Date | null }>(
      `SELECT persona_id, ended_at
       FROM sessions
       WHERE session_id = $1
       LIMIT 1`,
      [gif_session_id]
    ...
    executeSessionClose(
      {
        ...
        gif_session_id: args['gif_session_id'] as string,
      },
      persona,
      sessionId
    ),
