...
if (identityHmacSecret) {
  ...
  if (identityHmacSecretProblem) {
    throw new Error(
      `IDENTITY_HMAC_SECRET ${identityHmacSecretProblem}. Generate a strong ` +
...
if (!allowedOriginsResult.ok) {
  throw new Error(
    `${ALLOWED_ORIGINS_ENV} ${allowedOriginsResult.problem}. Set a comma-separated ` +
...
function createServer() {
  ...
  const server = new Server(
    { name: 'gif-mcp-server', version: '0.2.4' },
    {
      capabilities: { tools: {} },
    ...
    try {
      ...
      if (missingArg !== null) {
        throw new ProtocolError(
          ...
          `${missingArg} is required for ${name}`,
        );
      }
      ...
      inFlightCalls--;
      if (inFlightCalls === 0 && drainResolve) drainResolve();
    }
  });
