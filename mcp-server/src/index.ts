...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case 'db_read': {
      ...
      // TODO: Step 5 — implement db_read tool handler
      return {
        content: [{ type: 'text' as const, text: JSON.stringify({ status: 'not_implemented', tool: 'db_read' }) }],
        isError: true,
      };
    }
    ...
    case 'db_write': {
      ...
      // TODO: Step 6 — implement db_write tool handler
      return {
        content: [{ type: 'text' as const, text: JSON.stringify({ status: 'not_implemented', tool: 'db_write' }) }],
        isError: true,
      };
    }
